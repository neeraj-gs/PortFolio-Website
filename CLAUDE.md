...
- **Ledger** — The Edge section (`#edge`), the page's "why me" argument. Twenty-five AI-engineering disciplines in five `.ledger__group` sets: a *sticky* `.ledger__rail` on the left, numbered `.ledger__row`s on the right. An IntersectionObserver with a `-42%` band adds `.lit` to whichever rows cross the middle of the viewport, so scrolling reads like a playhead — no per-frame work for a 25-row list. `.edge` must never clip overflow (the rail is sticky). A closing panel of four differentiator statements was built here and cut: the ledger already makes the argument, and the panel only restated it.
